export function BrandNarrative() {
  return (
    <section className="py-32">
        <div className="text-center max-w-3xl mx-auto px-4">
            <h2 className="font-headline text-black text-xs tracking-[0.4em] uppercase mb-8">The Tigrafino Legacy</h2>
            <p className="font-headline text-3xl md:text-4xl text-black leading-tight italic">
              "Forged in heat, shaped by hand, and inspired by the relentless power of the tiger. TigraFINO creates artifacts for those who lead, not those who follow."
            </p>
        </div>
    </section>
  );
}